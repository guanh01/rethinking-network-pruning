...
parser.add_argument('data', metavar='DIR', default='/home/lning/PyTorch/rethinking-network-pruning/imagenet/data',
...
#         return
    ...
    compression_scheduler = distiller.file_config(model,optimizer, 
                                    '/home/lning/PyTorch/rethinking-network-pruning/imagenet/regularized_training/quant_aware_training.yaml', 
    ...
    regulate_param_init(model, weight_name_list, weight_list, Z_list, U_list, target)
...
#             change_idx_list_l = np.nonzero(torch.round(weight_scale * weight_flat) > 63)
#             change_idx_list_s = np.nonzero(torch.round(weight_scale * weight_flat) < -64)
...
def projection(weight, weight_scale, target):
    ...
    weight_flat = weight.view(-1)
    change_idx_list_l = np.nonzero(torch.round(weight_scale * weight_flat) > 63)
    change_idx_list_s = np.nonzero(torch.round(weight_scale * weight) < -64)
    change_idx_l_flat = change_idx_list_l.view(-1)
    change_idx_s_flat = change_idx_list_s.view(-1)
    overide_idx_l = np.nonzero(change_idx_l_flat % 8 != 7)
    overide_idx_s = np.nonzero(change_idx_s_flat % 8 != 7)
...
def regulate_param_init(model, weight_name_list, weight_list, Z_list, U_list, target):
    ...
    for param_tensor in model.state_dict():
        ...
        if 'float_weight' in name_part:
            ...
            Z_list.append(projection(Z, weight_scale, target))
            U = torch.zeros_like(Z)
            U_list.append(U)
            layer_id = layer_id + 1

def regulate_param_update(model, weight_name_list, weight_list, Z_list, U_list, target):
    weight_name_list = []
    weight_list = []
...
#             weight_zero_point = model.state_dict()[quantized_weight_zero_point]
            ...
            weight_list.append(weight)
            Z = weight + U_list_old[idx]
            Z = projection(Z, weight_scale, target)
#             Z_list.append(Z)
            U = U_list_old[idx] + weight - Z
            U_list.append(U)
            idx += 1
            
    return weight_list, Z_list, U_list
...
#             print(param_tensor, "\t", model.state_dict()[param_tensor].size())
            quantized_weight_name = param_tensor.replace('float_weight','')+'weight'
            quantized_weight_scale = param_tensor.replace('float_weight','')+'weight_scale'
#             print('here are the names: ', quantized_weight_name, quantized_weight_scale)
            
            float_weight = model.state_dict()[param_tensor]
#             print(model.state_dict()[quantized_weight_name])
            weight = model.state_dict()[quantized_weight_name]
            weight_scale = model.state_dict()[quantized_weight_scale]
            ...
            weight_flat = weight.view(-1)
            N = len(weight_flat)
#             print(N, weight_flat.size(), weight.size())
            total += N
...
#         print(i)
        ...
        # add quantization and regularization
...
#         loss = criterion(output, target)
        loss = regulate_loss(output, target,criterion).cuda(args.gpu)
        ...
        loss.backward()
        optimizer.step()
        compression_scheduler.on_minibatch_end(epoch, i, 10010)
        if i%2000 == 0:
            check_large_weights_count(model)
            # evaluate teh model
            print('evaluation before regularization')
            acc1 = validate(val_loader, model, criterion, args)
            regulate_quantized_weight(model)
            check_large_weights_count(model)
            print('evaluation after regularization')
        ...
        if i % args.print_freq == 0:
            progress.print(i)
            print('batch time: ', batch_time_total)
            avg_time = time.time() - avg_time
            print('avg_batch_time: ', avg_time/args.print_freq)
            avg_time = time.time()
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        print(' Evaluation: * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}'
              .format(top1=top1, top5=top5))

    return top1.avg

def save_checkpoint(state, is_best, filename='/home/lning/PyTorch/rethinking-network-pruning/imagenet/regularized_training/model/alpha_0-00005_beta_0-0015_lr_0-0001/checkpoint.pth.tar'):

...
#     indexes = random.choice(tensor.nelement(), size=n_bits)
    ...
    for index in indexes:
        ...
        tensor[vid] = value_after_flip   
        if debug_mode:
            print('after flip, value: %10d (%s), bid: %d' %(value_after_flip, bits.bin, bid)) 
...
if __name__ == '__main__':
    tensor = torch.randint(-128, 128, size=(5,2)).cuda()
    random = np.random
    bit_position = 1 
    n_bits = 5
    print('before:', tensor)
    inject_faults_int8_random_bit_position(tensor, random, n_bits, debug_mode=True)
    print('after: ', tensor)